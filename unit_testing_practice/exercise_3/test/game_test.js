...
describe('GAME INSTANCE FUNCTIONS', function () {
  ...
  describe('saveGame', function () {
    // pass 'done' to the test's callback to make it async aware
    it('shold update save status', function (done) {
      var status = 'game not saved...'

      saveGame(function () {
        status = 'game saved!'
        expect(status).to.equal('game saved!')
        // Simply passing done() isn't enough, we need to call it once
        // async actions are completed to avoid Mocha timeout
        done()
      })
    })
  })
